...
for(let i=1;i<=10;i++){const sw=node.widgets?.find((w)=>w.name===`switch_${i}`);if(sw){const origCb=sw.callback;sw.callback=function(){origCb?.apply(this,arguments);updateVisibility();};}}
