...
const modeBarWidget=node.addDOMWidget('_ls_mode','custom',bar,{getValue:()=>currentMode,setValue:(v)=>{if(MODE_OPTIONS.includes(v)){currentMode=v;for(const c of chipEls)c.classList.toggle('selected',c.textContent===currentMode);}},getMinHeight:()=>26,getMaxHeight:()=>26,serialize:false,});modeBarWidget.computeLayoutSize=()=>({minHeight:26,minWidth:0,maxHeight:26});const newIdx=node.widgets.indexOf(modeBarWidget);if(newIdx>=0&&newIdx!==origIdx){node.widgets.splice(newIdx,1);node.widgets.splice(origIdx,0,modeBarWidget);}
