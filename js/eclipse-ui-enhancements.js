...
app.registerExtension({
  ...
  async init(app) {
    ...
    try {
      const response = await fetch("/eclipse/config/log_level");
      if (response.ok) {
        const data = await response.json();
        currentLogLevel = data.log_level || "warning";
      }
    } catch (error) {
      console.error("[Eclipse] Failed to fetch log level:", error);
    }
      ...
      async onChange(value) {
        try {
          ...
          if (response.ok) {
            const data = await response.json();
            if (data.success) {
              console.log(`[Eclipse] Log level changed to: ${value}`);
            } else {
              console.error("[Eclipse] Failed to update log level:", data.error);
            }
          } else {
            console.error("[Eclipse] Server error updating log level:", response.status);
          }
        } catch (error) {
          console.error("[Eclipse] Failed to update log level:", error);
        }
      },
    });
  },
...
app.registerExtension({
  ...
  async init(app) {
    ...
    try {
      const response = await fetch("/eclipse/config/dev_mode");
      if (response.ok) {
        const data = await response.json();
        currentDevMode = data.dev_mode || false;
      }
    } catch (error) {
      console.error("[Eclipse] Failed to fetch dev mode:", error);
    }
    ...
    app.ui.settings.addSetting({
      ...
      defaultValue: currentDevMode,
      async onChange(value) {
        try {
          const response = await fetch("/eclipse/config/dev_mode", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ dev_mode: value }),
          });
          
          if (response.ok) {
            const data = await response.json();
            if (data.success) {
              console.log(`[Eclipse] Dev mode ${value ? 'enabled' : 'disabled'}`);
            } else {
              console.error("[Eclipse] Failed to update dev mode:", data.error);
            }
          } else {
            console.error("[Eclipse] Server error updating dev mode:", response.status);
          }
        } catch (error) {
          console.error("[Eclipse] Failed to update dev mode:", error);
        }
      },
    });
  },
