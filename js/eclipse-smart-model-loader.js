/* eclipse-smart-model-loader.js - Multi-select feature widget + visibility for Smart Model Loader [Eclipse] */
...
const FEATURE_OPTIONS = [
    'templates', 'clip', 'vae', 'latent', 'sampler',
...
const SEED_VAL_TO_MODE_CHIP = Object.fromEntries(
    Object.entries(SEED_MODE_CHIP_TO_VAL).map(([k, v]) => [v, k])
);

// Radio groups: only one seed mode at a time
const RADIO_GROUPS = [SEED_MODE_CHIPS];

// All mode chips (UI-only, stripped from prompt)
const ALL_MODE_CHIPS = new Set(SEED_MODE_CHIPS);
...
// Dynamic template button name
const TEMPLATE_BUTTON = '_btn_template_action';

// Seed buttons (added dynamically)
const SEED_BUTTONS = ['_btn_last_seed'];

// All controlled widgets
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, _app) {
        ...
        nodeType.prototype.onNodeCreated = function () {
                ...
                try {
                    ...
                    const updateList = (widgetName, newValues) => {
                        ...
                        if (!newValues.includes(w.value)) {
                            ...
                            if (fwd !== w.value && newValues.includes(fwd)) { w.value = fwd; }
                            else {
                    ...
                    if (data.vae) updateList('vae_name', data.vae);
            ...
            // --- Template: reset all fields to defaults ---
            ...
            const loadTemplateData = async (name) => {
                ...
                try {
                    const ts = Date.now();
                    const resp = await fetch(`/eclipse/loader_templates/${name}.json?t=${ts}`, { cache: 'no-store' });
            ...
            const applyTemplate = async (name) => {
                ...
                if (!data) { updateVisibility(); return; }

                // Preserve features that are never saved/loaded from templates
                ...
                const prevSeedMode = prevFeats.find(f => ALL_MODE_CHIPS.has(f));

                isLoadingTemplate = true;
                try {
                    // Reset to defaults first, then overlay template values
                    resetAllFields();

                    // Set features from template (map old boolean toggles to new features)
                    ...
                    // Also check for new-style features array in template
                    let newFeatures;
                    if (data.features && Array.isArray(data.features)) {
                        newFeatures = data.features.filter(f => f !== 'templates' && f !== 'memory_cleanup' && f !== 'seed' && !ALL_MODE_CHIPS.has(f));
                    } else {
                        newFeatures = templateFeatures;
                    }
                    // Restore features that are never saved/loaded from templates
                    ...
                    if (prevSeedMode && !newFeatures.includes(prevSeedMode)) {
                        newFeatures.push(prevSeedMode);
                    }
                    featWidget.value = newFeatures;

                    // Apply all template values
                    const fields = [
                        'model_type', 'weight_dtype', 'blocks_to_swap', 'offload_embeddings',
                        'sampling_method', 'sampling_subtype', 'shift', 'base_shift',
                        'sampling_width', 'sampling_height', 'original_timesteps', 'zsnr',
                        'sigma_max', 'sigma_min', 'data_type', 'cache_threshold', 'attention',
                        'i2f_mode', 'cpu_offload', 'num_blocks_on_gpu', 'use_pin_memory',
                        'gguf_dequant_dtype', 'gguf_patch_dtype', 'gguf_patch_on_device',
                        'clip_source', 'clip_count', 'clip_name1', 'clip_name2', 'clip_name3',
                        'clip_name4', 'clip_type', 'enable_clip_layer', 'stop_at_clip_layer',
                        'vae_source', 'vae_name', 'resolution', 'width', 'height', 'batch_size',
                        'lora_count', 'ckpt_name', 'unet_name', 'nunchaku_name', 'qwen_name',
                        'zimage_name', 'gguf_name',
                    ];
            ...
            const handleTemplateAction = async () => {
                ...
                if (action === 'None') {
                    ...
                    // Build config from current widget state
            ...
            const handleTemplateDelete = async () => {
                ...
                try {
                    const resp = await api.fetchApi('/eclipse/loader_templates/delete', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ name: tmplName }),
                    });
                    const result = await resp.json();
                    if (result.success) {
                        broadcastTemplateListChanged(await refreshTemplateList(), node.id);
            ...
            // --- Build template config from current state ---
            const buildTemplateConfig = () => {
                const cfg = {};
                const raw = vis.getValue('features');
                const feats = Array.isArray(raw) ? raw : [];
                cfg.features = feats.filter(f => f !== 'templates' && f !== 'memory_cleanup' && f !== 'seed' && !ALL_MODE_CHIPS.has(f));

                const mt = gv('model_type');
                cfg.model_type = mt;

                // Map features to old-style booleans for backward compat
                ...
                if (mt === 'Standard Checkpoint') {
                    const v = gv('ckpt_name'); if (v && v !== 'None') cfg.ckpt_name = v;
                } else if (mt === 'UNet Model') {
                    const v = gv('unet_name'); if (v && v !== 'None') cfg.unet_name = v;
                    cfg.weight_dtype = gv('weight_dtype');
                } else if (mt === 'Nunchaku Flux') {
                    const v = gv('nunchaku_name'); if (v && v !== 'None') cfg.nunchaku_name = v;
                    ...
                    const v = gv('qwen_name'); if (v && v !== 'None') cfg.qwen_name = v;
                    cfg.cpu_offload = gv('cpu_offload');
                    cfg.num_blocks_on_gpu = gv('num_blocks_on_gpu');
                    cfg.use_pin_memory = gv('use_pin_memory');
                } else if (mt === 'Nunchaku ZImage') {
                    const v = gv('zimage_name'); if (v && v !== 'None') cfg.zimage_name = v;
                    cfg.cpu_offload = gv('cpu_offload');
                    cfg.num_blocks_on_gpu = gv('num_blocks_on_gpu');
                    cfg.use_pin_memory = gv('use_pin_memory');
                } else if (mt === 'GGUF Model') {
                    const v = gv('gguf_name'); if (v && v !== 'None') cfg.gguf_name = v;
                ...
                if (feats.includes('latent')) {
                    ...
                    cfg.resolution = res;
                    if (res === 'Custom') { cfg.width = gv('width'); cfg.height = gv('height'); }
                }
                ...
                if (feats.includes('model_sampling')) {
                    ...
                    if (sm === 'Flux') { cfg.sampling_width = gv('sampling_width'); cfg.sampling_height = gv('sampling_height'); }
                    else if (sm === 'LCM') { cfg.original_timesteps = gv('original_timesteps'); cfg.zsnr = gv('zsnr'); }
                    else if (sm === 'ContinuousEDM') { cfg.sampling_subtype = gv('sampling_subtype'); cfg.sigma_max = gv('sigma_max'); cfg.sigma_min = gv('sigma_min'); }
                    else if (sm === 'ContinuousV') { cfg.sigma_max = gv('sigma_max'); cfg.sigma_min = gv('sigma_min'); }
                }
                return cfg;
            };
            ...
            const updateVisibility = () => {
                ...
                // --- Filter model file extensions per type ---
                const modelFilter = {
                    ckpt_name: { show: isStd, exts: ['.safetensors', '.ckpt', '.pt', '.bin', '.sft'] },
                    unet_name: { show: isUnet, exts: ['.safetensors', '.pt', '.bin', '.sft'] },
                    nunchaku_name: { show: isNFlux, exts: ['.safetensors', '.pt', '.bin', '.sft'] },
                    qwen_name: { show: isNQwen, exts: ['.safetensors', '.pt', '.bin', '.sft'] },
                    zimage_name: { show: isNZimg, exts: ['.safetensors', '.pt', '.bin', '.sft'] },
                    gguf_name: { show: isGGUF, exts: ['.gguf'] },
                };
                ...
                if (showButton && !templateButton) {
                    ...
                    if (templateButton.name !== label) { templateButton.name = label; notifyVue(node); }
                ...
                // --- CLIP widgets (conditional sub-visibility) ---
                const hasClip = feats.has('clip');
                const clipExternal = gv('clip_source') === 'External';
                ...
                // --- Seed widgets ---
                const seedVisible = feats.has('seed') || SEED_MODE_CHIPS.some((c) => feats.has(c));
                d('seed', seedVisible);
            ...
            if (featWidget) {
                featWidget.callback = function (value) {
                    ...
                    // Sync to hidden backing widget WITHOUT mode chips (mode chips are UI-only)
                    ...
                    // When templates feature is disabled, reset template state
                    // to prevent stale Load action from re-applying on page refresh
                    ...
                    // Sync mode chips → seed widget value
                    const selectedSet = new Set(feats);
                    if (node._Eclipse_seedWidget) {
                        const activeMode = SEED_MODE_CHIPS.find((m) => selectedSet.has(m));
                        if (activeMode) {
                            // Mode chip selected → ensure seed parent chip is also selected
                            if (!selectedSet.has('seed')) {
                                selectedSet.add('seed');
                            }
                            const modeVal = SEED_MODE_CHIP_TO_VAL[activeMode];
                            if (node._Eclipse_seedWidget.value !== modeVal) {
                                node._Eclipse_seedWidget.value = modeVal;
                                node._Eclipse_seedWidget.callback?.(modeVal);
                            }
                        }
                        // Write back chip changes if seed was auto-added
                        if (selectedSet.size !== feats.length) {
                            node._Eclipse_updatingChips = true;
            ...
            // --- Hook widget change callbacks for visibility triggers ---
            const triggerWidgets = [
                'template_action', 'template_name', 'model_type',
                'sampling_method', 'clip_source', 'clip_count', 'clip_type',
                'vae_source', 'resolution', 'lora_count',
            ];
            for (const wName of triggerWidgets) {
                ...
                w.callback = function () {
                    ...
                    if (wName === 'sampling_method') {
                        ...
                        const defaults = { SD3: 3, AuraFlow: 1.73, Flux: 1.15, 'Stable Cascade': 2, LTXV: 2.05 };
                        if ((Object.values(defaults).some(v => Math.abs(curShift - v) < 0.01) || curShift === 3) && defaults[sm]) {
                            sv('shift', defaults[sm]);
                        }
                        if (sm === 'ContinuousEDM') { sv('sigma_max', 120); sv('sigma_min', 0.002); }
                        else if (sm === 'ContinuousV') { sv('sigma_max', 500); sv('sigma_min', 0.03); }
                    }
            ...
            // --- Listen for template changes from other nodes ---
            const onTemplateChanged = (e) => {
                const { templates, sourceNodeId } = e.detail;
            ...
            // --- onConfigure (loading saved workflows) ---
    ...
    async setup() {
        // Mode switch: recreate nodes for correct widget path
        onVueModeChange(() => {
        ...
        app.graphToPrompt = async function () {
            ...
            for (const node of nodes) {
                ...
                // Strip mode chips from features — they're UI-only
