...
export function isSetterActive(graph,setter){if(!setter)return false;if(setter.mode===2||setter.mode===4)return false;if(!setter.inputs?.[0]?.link)return false;let link=getLink(graph,setter.inputs[0].link);if(!link)return false;let originNode=graph.getNodeById?.(link.origin_id);while(originNode&&originNode.mode===4){const outType=originNode.outputs?.[link.origin_slot]?.type;let matchedInput=null;if(originNode.inputs){for(const inp of originNode.inputs){if(inp.link!=null&&(inp.type===outType||inp.type==='*'||outType==='*')){matchedInput=inp;break;}}}
...
export function resolveBypassedLink(graph,setter){if(!setter?.inputs?.[0]?.link)return null;let link=getLink(graph,setter.inputs[0].link);if(!link)return null;let originNode=graph.getNodeById?.(link.origin_id);while(originNode&&originNode.mode===4){const outType=originNode.outputs?.[link.origin_slot]?.type;let matchedInput=null;if(originNode.inputs){for(const inp of originNode.inputs){if(inp.link!=null&&(inp.type===outType||inp.type==='*'||outType==='*')){matchedInput=inp;break;}}}
