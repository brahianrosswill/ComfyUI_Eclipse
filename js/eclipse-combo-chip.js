...
applyTriggerWidth();function updateLabel(){const n=selectedSet.size;trigger.innerHTML=`<span>${n} item${n !== 1 ? 's' : ''} selected</span><span class="arrow">▼</span>`;}
...
selectedSet=new Set(arr.filter((x)=>featureOptions.includes(x)&&!disabledSet.has(x)));updateLabel();if(panel){for(const c of panel.children){c.classList.toggle('selected',selectedSet.has(c.textContent));c.classList.toggle('disabled',disabledSet.has(c.textContent));}}},getMinHeight:()=>WIDGET_TOTAL_HEIGHT,getMaxHeight:()=>WIDGET_TOTAL_HEIGHT,margin:WIDGET_MARGIN,};if(!serialize)widgetOpts.serialize=false;featWidget=node.addDOMWidget(widgetName,'custom',trigger,widgetOpts);featWidget.computedHeight=WIDGET_TOTAL_HEIGHT;featWidget.computeLayoutSize=()=>({minHeight:WIDGET_TOTAL_HEIGHT,minWidth:0,maxHeight:WIDGET_TOTAL_HEIGHT});const unsubMode=onVueModeChange(applyTriggerWidth);const origOnRemove=featWidget.onRemove;featWidget.onRemove=function(){closePanel();unsubMode();origOnRemove?.call(this);};const newIdx=node.widgets.indexOf(featWidget);if(newIdx>=0&&newIdx!==origIdx){node.widgets.splice(newIdx,1);node.widgets.splice(origIdx,0,featWidget);}
