...
const rect=trigger.getBoundingClientRect();const PANEL_WIDTH=320;panel.style.left=`${rect.left}px`;panel.style.top=`${rect.bottom + 2}px`;panel.style.width=`${PANEL_WIDTH}px`;document.body.appendChild(panel);const pr=panel.getBoundingClientRect();const vw=window.innerWidth;const vh=window.innerHeight;if(pr.right>vw){panel.style.left=`${Math.max(0, vw - pr.width - 4)}px`;}
...
const unsubMode=onVueModeChange(applyTriggerWidth);const origOnRemove=featWidget.onRemove;featWidget.onRemove=function(){closePanel();unsubMode();origOnRemove?.call(this);};const newIdx=node.widgets.indexOf(featWidget);if(newIdx>=0&&newIdx!==origIdx){node.widgets.splice(newIdx,1);node.widgets.splice(origIdx,0,featWidget);}
